...
# 处理缺失值：删除包含缺失值的行（外部验证集的标准做法）
original_len = len(val_data)
val_data = val_data.dropna(subset=selected_features + [target])
dropped_count = original_len - len(val_data)
if dropped_count > 0:
    print(f"⚠️ 警告: 验证集中发现 {dropped_count} 行数据包含缺失值，已自动剔除。")
...
def plot_roc(y_true, y_probs, save_path):
    ...
    plt.title('ROC Curve - External Validation', fontsize=14)
...
def plot_cic(y_true, y_probs, save_path):
    ...
    plt.title('Clinical Impact Curve', fontsize=14)
    plt.legend(loc='upper right', fontsize=11)
    plt.grid(alpha=0.3)
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
    plt.close()
...
plot_roc(y_val, y_probs, f'{output_dir}/1_ROC_Validation.png')
plot_calibration(y_val, y_probs, f'{output_dir}/2_Calibration_Validation.png')
plot_dca(y_val, y_probs, f'{output_dir}/3_DCA_Validation.png')
plot_cic(y_val, y_probs, f'{output_dir}/4_CIC_Validation.png')

print(f"\n所有外部验证分析图已保存至文件夹: {output_dir}")
